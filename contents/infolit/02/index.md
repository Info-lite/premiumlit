...
[教室パソコンの利用方法]: ../01/index.html#chapter10 "教室パソコンの利用方法"
...
#### ログイン
  ...
  1. [kibaco](https://kibaco-d.tmu.ac.jp/login)にアクセスし、[ログイン]ボタンを押してTMU ID（学修番号@ed.tmu.ac.jp）でログインします。パスワードは、[教室パソコンの利用方法][]で説明されている「パスワード」と同じです。
    {% screenshot login3.png "kibacoログイン TMUID" %}
  2. ログインに成功すると、トップページが表示されます。
    ...
    {% screenshot login5.png "kibacoマイページ" %}
...
115室の一部の端末では Mac 環境を利用することもできます。
...
たとえば、Windows 環境で作成して<a href="./#chapter23">マイドキュメント</a>に保存したデータを Mac 環境で編集すること、あるいはその逆の操作をすることができます。</p>
...
後述の利用者メニューに無線LAN接続のためのネットワーク名（SSID）が記載されています。 
この情報を用いてパソコンやスマートフォンで無線LANが利用できるよう設定します。

初めて接続するときは認証画面が表示されます。
[教室パソコンの利用方法][]で説明されている「ユーザー名」と「パスワード」を入力して接続します。
同じ機器であれば、以降は自動的に接続されるようになります。
複数の機器を無線LANに接続したいときは、それぞれの機器で同じように設定を行ってください。
...
そのほか、教育研究用情報システムのパスワード変更も利用者メニューで行います。
  ...
  3. 画面に各種メニューが表示されます。パスワードの変更は、画面上部のメニューの "パスワード等" をクリックし、"パスワード変更" を選択します。
...
##### kibacoへのファイルアップロード
[資料配布機能](./#chapter9)の「ファイルをアップロードする」をご覧ください。
{% comment %}
    1. まず、「資料」を選択します。
